...
import { getOrCreateReport, saveReportFields, Report5110_28Data } from "@/lib/reportData";
...
const Report5110_28 = () => {
  ...
  const loadReport = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await getOrCreateReport<Report5110_28Data>(organizationId, user.id, '5110-28', reportPeriod);
      setReportData(data);
      const { data: ops, error } = await supabase
        .from('operations')
        .select('id, operation_date, type, proof, liters, proof_gallons')
        .eq('organization_id', organizationId)
        .in('type', ['bottling', 'tax_withdrawal'])
  ...
  const updateReportField = async (field: keyof Report5110_28Data, value: any) => {
    if (!reportData || !organizationId || !user) return;
    setReportData({ ...reportData, [field]: value } as Report5110_28Data);
    try {
      const saved = await saveReportFields<Report5110_28Data>(organizationId, user.id, '5110-28', reportPeriod, { [field]: value } as any);
      setReportData(saved);
    } catch (err: any) {
  ...
  const handleDownloadPDF = () => {
    toast.success('Downloading TTB Form 5110.28', { description: `TTB_5110_28_${format(reportPeriod, 'yyyy-MM')}.pdf` });
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Monthly Report of Processing Operations</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center"><FileText className="mr-2 h-5 w-5" />Monthly Report of Processing Operations (Form 5110.28)</CardTitle>
          ...
          <Tabs defaultValue="report">
            ...
            <TabsContent value="info">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2"><Label>Processing Facility Address</Label><Input value={reportData?.proprietorAddress || ''} onChange={(e) => updateReportField('proprietorAddress', e.target.value)} /></div>
              ...
              <div className="border rounded-md overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    <tr className="border-b"><td className="p-3">2. Bottling production</td><td className="p-3 text-right">{inv?.bottling.toFixed(1) ?? '0.0'}</td></tr>
                    <tr className="border-b bg-muted/20"><td className="p-3 font-medium">3. Total (1 + 2)</td><td className="p-3 text-right font-medium">{inv ? (inv.beginningInventory + inv.bottling).toFixed(1) : '0.0'}</td></tr>
                    <tr className="border-b"><td className="p-3">4. Tax withdrawals</td><td className="p-3 text-right">{inv?.taxWithdrawal.toFixed(1) ?? '0.0'}</td></tr>
                    <tr className="border-b bg-muted/50"><td className="p-3 font-bold">5. Ending inventory (3 - 4)</td><td className="p-3 text-right font-bold">{inv?.endingInventory.toFixed(1) ?? '0.0'}</td></tr>
                  </tbody>
                </table>
              </div>
            </TabsContent>
